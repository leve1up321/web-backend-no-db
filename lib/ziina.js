...
/**
 * التحقق من صحة توقيع Webhook باستخدام HMAC SHA256
 * @param {string} payloadText - محتوى الـ webhook كنص
 * @param {string} signatureHeader - التوقيع من الـ header
 * @param {string} secret - المفتاح السري (ZIINA_WEBHOOK_SECRET)
 * @returns {boolean} - هل التوقيع صحيح؟
 */
export function verifyWebhookSignature(payloadText, signatureHeader, secret = null) {
  try {
    // استخدام السر من المعامل أو من متغيرات البيئة
    const webhookSecret = secret || process.env.ZIINA_WEBHOOK_SECRET;
    
    if (!webhookSecret) {
      console.error('❌ ZIINA_WEBHOOK_SECRET غير موجود');
      return false;
    }
    
    if (!signatureHeader) {
      console.error('❌ التوقيع غير موجود في الـ header');
      return false;
    }
...
export const createPaymentIntent = (paymentData) => ziinaGateway.createPaymentIntent(paymentData);
export const getPaymentStatus = (paymentId) => ziinaGateway.getPaymentStatus(paymentId);
export const processWebhook = (webhookData) => ziinaGateway.processWebhook(webhookData);
export const refundPayment = (paymentId, amount = null) => ziinaGateway.refundPayment(paymentId, amount);
...
// تصدير CommonJS (محذوف لصالح ES6)
